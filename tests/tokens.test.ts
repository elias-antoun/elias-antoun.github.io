...
function luminance(hex: string): number {
  const channels = hex.replace('#', '').match(/\w\w/g)!.map((h) => {
...
describe('WCAG AA contrast in both palettes', () => {
  ...
  const PAIRS: Array<[string, string]> = [
    ...
    ['--accent', '--bg'],
    ['--accent', '--surface'],
    // Primary buttons and the skip link paint text on an accent fill. White
    // here scored 2.69:1 in dark mode, which is why --accent-contrast exists.
  ...
  it('flags plain white on the accent as insufficient in dark mode', () => {
    // Regression guard documenting the original bug: if someone reverts
    // --accent-contrast to white, this records why that fails.
